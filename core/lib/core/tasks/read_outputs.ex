defmodule Core.Tasks.ReadOutputs do
  ...
  def execute(%DB.Task{device: device}, status) do
    with {:ok, data} <- Basics.read_outputs(device) do
  ...
  defp update_out_of_date(device, high, low) do
    ...
    if a == 0 do
      ...
      Enum.each(res, fn {type, id, ref} -> Channel.broadcast_change(type, id, ref) end)
      Logger.info(
        "Updated '#{a}' ports belonging to the device with id: '#{device.id}', name: '#{device.name}'"
      )
    end
  end
end
