  ...
  def report(device_id, hours \\ 3) do
    ...
    from(
      ...
      where: 
        ...
        and fragment("cast(strftime('%s', ?) as Integer) >= strftime('%s', 'now') - ?", d.inserted_at, ^seconds),
      group_by: 
        fragment("cast((julianday('2019-01-01T00:00:00') - julianday(?))*24*60 As Integer) / 5", d.inserted_at),
      select: %{value: count(d.id), 
        date: fragment("datetime(strftime('%s', ?) - strftime('%s', ?) % 300, 'unixepoch', 'localtime')", d.inserted_at, d.inserted_at)}
    )
    |> Repo.all()
  end
  ...
  def report_s(device_id, hours \\ 3) do
    ...
    from(
      ...
      group_by: 
        fragment("cast((julianday('2019-01-01T00:00:00') - julianday(?))*24*60 As Integer) / 5", d.inserted_at),
      select: %{value: count(d.id), 
        date: fragment("strftime('%s', 'now') - (strftime('%s', ?) - strftime('%s', ?) % 300)", d.inserted_at, d.inserted_at)}
    )
    |> Repo.all()
  end
