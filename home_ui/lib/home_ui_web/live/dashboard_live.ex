defmodule HomeUiWeb.DashboardLive do
  ...
  def mount(_params, _session, socket) do
    ...
    pages = DB.Page.short_info()
    {:ok, assign(socket, pages_short: pages)}
  end

  def handle_params(%{"page_id" => page_id}, _url, socket) do
    {id, _} = Integer.parse(page_id)
    page = DB.Page.get(id) |> DB.Repo.preload() 
    if !is_nil(page) do
      lights = DB.Page.lights(page)
      dimmers = DB.Page.dimmers(page) |> Enum.map(&DB.Port.Dimmer.preload_lights/1)
      sunblinds = DB.Page.sunblinds(page)
      sensors = DB.Page.sensors(page)
      plan = get_plan(id)

      {:noreply,
       assign(socket, lights: lights, dimmers: dimmers, sunblinds: sunblinds, sensors: sensors, plan: plan)}
    else
      {:noreply, socket}
    end
  end
  ...
  def handle_params(_params, _url, socket) do
    {id, _} = hd(socket.assigns[:pages_short])
    ...
    items =
      Enum.map(items, fn port ->
        if(port.id == dimmer_id) do
          ...
          dimmer = Port.cast(port, [more: [lights: lights]])
          broadcast_from_self!(%{type: :dimmer, item: dimmer})
          dimmer
        else
          port
